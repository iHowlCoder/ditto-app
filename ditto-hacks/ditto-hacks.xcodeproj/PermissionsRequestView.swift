            ...
            VStack(spacing: 32) {
                ...
                VStack(spacing: 16) {
                    ...
                    Button(action: {
                        if permissionsManager.photoLibraryPermissionStatus == .notDetermined {
                            Task {
                                print("📚 Photo Library row tapped - requesting permission")
                                await permissionsManager.requestPhotoLibraryPermission()
                            }
                        } else if permissionsManager.photoLibraryPermissionStatus == .denied {
                            print("⚙️ Photo Library denied - opening settings")
                            permissionsManager.openAppSettings()
                        }
                    }) {
                        PermissionRow(
                    ...
                    .buttonStyle(PlainButtonStyle())
                    .disabled(permissionsManager.photoLibraryPermissionStatus == .authorized)
                }
                ...
                VStack(spacing: 12) {
                    if allPermissionsGranted {
                        ...
                        // Help text
                        Text("Tap each permission above to enable it")
                        ...
                        Button(action: {
                            print("🔓 Grant All Permissions button tapped")
                            requestPermissions()
                        }) {
                            HStack {
    ...
    private func requestPermissions() {
        Task { @MainActor in
            ...
            if permissionsManager.cameraPermissionStatus == .notDetermined {
                ...
                print("📷 Camera permission already determined: \(permissionsManager.cameraPermissionStatus)")
            }
            ...
            if permissionsManager.photoLibraryPermissionStatus == .notDetermined {
                ...
                print("📚 Photo library permission already determined: \(permissionsManager.photoLibraryPermissionStatus)")
            }
            ...
            print("✅ Permission request flow complete")
            ...
            if permissionsManager.cameraPermissionStatus == .denied ||
               permissionsManager.photoLibraryPermissionStatus == .denied {
                print("⚠️ Some permissions were denied, showing settings alert")
